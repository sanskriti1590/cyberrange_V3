...
const Network = ({ handleClick }) => {
  ...
  const validateInput = (name, value) => {
    ...
    if (name === 'subnet_cidr' && !validateCIDR(value)) {
  ...
  const handleSubmit = (event) => {
    ...
    if (network.subnet_cidr.trimLeft() === '') {
      newErrors.subnet_cidr = 'CIDR ip cannot be empty';
    }
    ...
    <Stack margin={5} gap={4}>
      <Stack gap={2}>
        ...
        <Typography variant="body1" color="#acacac !important">
          In a Cyber Range CTF, network challenges are a crucial component that focuses on assessing participants' knowledge and skills related to network security, protocols, and analysis. These challenges simulate real-world scenarios and require participants to analyze and manipulate network traffic, identify vulnerabilities, and protect network infrastructure. Here are some examples of network challenges that can be included:
        </Typography>
      </Stack>
      ...
      <Stack width="100%">
        <TextField
          ...
          onChange={changeHandler}
        />
        {errors.subnet_cidr && <span className="error">{errors.subnet_cidr}</span>}
      </Stack>
      <Box gap={2}>
