...
import { endGame, getConsoleDetailsSenario, getScenarioIpAddress, submitFlag } from "../../../APIConfig/scenarioConfig";
import './index.css'
               ...
               <Stack gap={3} sx={{ position: 'sticky', top: 0 }}>
                        ...
                        >

                           {
                              ip?.map((item, index) => {
                                 return (
                                    <Stack direction='row' justifyContent='start' alignItems='center'
                                       borderBottom='1px solid #EAEAEB' padding={1}>
                                       <Typography variant='h5' sx={{ color: '#EAEAEB !important' }}>
                                          {item?.ip_address}
                                       </Typography>
                                       <Typography variant='h5' mx={1} sx={{ color: '#EAEAEB !important' }}>
                                          -
                                       </Typography>
                                       <Typography variant='h5' sx={{ color: '#EAEAEB !important' }}>
                                          {item?.instance_name}
                  ...
                  <Stack p={2} gap={2} sx={{ backgroundColor: '#16181F', borderRadius: '12px' }}>
                     <Stack direction='row' justifyContent='start' alignItems='center' spacing={1}>
                        ...
                        <Typography variant='h4' sx={{ color: '#BCBEC1 !important' }}>{data?.scenario_vm_username}</Typography>
                     </Stack>

                     <Stack direction='row' justifyContent='start' alignItems='center' spacing={1}>
                        <Icons.key style={{ fontSize: '24px', color: '#BCBEC1' }} />
                        <Typography variant='h4' sx={{ color: '#BCBEC1 !important' }}>Password : </Typography>
                        <Typography variant='h4' sx={{ color: '#BCBEC1 !important' }}>{data?.scenario_vm_password}</Typography>
                     </Stack>
                     ...
                     <Typography variant='h3' sx={{ fontSize: '20px !important' }}> {data?.scenario_name}</Typography>
                     ...
                     <Stack direction='row' justifyContent='start' alignItems='center' spacing={2}>
                        <Stack direction='row' justifyContent='start' alignItems='center' spacing={0.5}>
                           ...
                           <Typography variant='h5'
                              sx={{ color: '#BCBEC1 !important' }}>{data?.scenario_assigned_severity}</Typography>
                        </Stack>
                        <Stack direction='row' justifyContent='start' alignItems='center' spacing={0.5}>
                           <Icons.userCheck style={{ fontSize: '24px', color: '#BCBEC1' }} />
                           <Typography variant='h5'
                              sx={{ color: '#BCBEC1 !important' }}>{data?.scenario_player_count}</Typography>
                        </Stack>
                        <Typography
                           variant='h5'
                           sx={{
                     ...
                     </Stack>
                     {
                        ...
                        (<CustomModal open={openModal} onClose={closeModalHandler}>
                           <Box sx={{ color: '#BCBEC1', height: '560px', overflow: 'auto' }}
                              dangerouslySetInnerHTML={{ __html: data?.scenario_description }} />
                        </CustomModal>)
                     }
                     <Box sx={{ maxHeight: '300px', overflow: 'auto' }}>
                        <Box sx={{ color: '#BCBEC1 ' }} dangerouslySetInnerHTML={{ __html: data?.scenario_description }} />
                     </Box>
                  </Stack>
               </Stack>
            </Stack>
